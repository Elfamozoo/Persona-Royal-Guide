...
import { Search, Plus, Equal, Sparkles, ArrowLeftRight, ListFilter, Info } from 'lucide-react';
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 w-full">
                <div className="flex-1 w-full text-center space-y-2">
                  <div className={`h-24 flex items-center justify-center border-4 border-dashed ${p1 ? 'border-p5-red bg-p5-red/10' : 'border-white/10'}`}>
                    {p1 ? (
                      <div>
                        <div className="font-black italic uppercase">{translatePersona(p1.name)}</div>
                        <div className="text-[10px] font-bold text-p5-red uppercase">{translateArcana(p1.arcana)}</div>
                      </div>
                    ) : <div className="text-white/10 font-black italic uppercase">P1</div>}
                  </div>
                ...
                <div className="flex-1 w-full text-center space-y-2">
                  <div className={`h-24 flex items-center justify-center border-4 border-dashed ${p2 ? 'border-p5-red bg-p5-red/10' : 'border-white/10'}`}>
                    {p2 ? (
                      <div>
                        <div className="font-black italic uppercase">{translatePersona(p2.name)}</div>
                        <div className="text-[10px] font-bold text-p5-red uppercase">{translateArcana(p2.arcana)}</div>
                      </div>
                    ) : <div className="text-white/10 font-black italic uppercase">P2</div>}
                  </div>
                ...
                <div className="flex-1 w-full text-center">
                  <div 
                    onClick={() => forwardResult && setSelectedPersona(forwardResult)}
                    className={`h-24 flex flex-col items-center justify-center border-4 transition-all ${forwardResult ? 'bg-white text-p5-black border-p5-red scale-110 shadow-lg cursor-pointer hover:bg-p5-red hover:text-white' : 'border-white/10'}`}
                  >
                    {forwardResult ? (
                      <>
                        <div className="font-black italic uppercase leading-none">{translatePersona(forwardResult.name)}</div>
                    ...
                    >
                      ...
                      <td className="p-2">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-black uppercase italic text-sm">{translatePersona(p.name)}</span>
                          <button 
                            onClick={(e) => { e.stopPropagation(); setSelectedPersona(p); }}
                            className="p-1 hover:bg-white hover:text-p5-black rounded transition-colors"
                          >
                            <Info size={14} />
                          </button>
                        </div>
                      ...
                      {p.elems.map((res, i) => (
                        <td key={i} className={`p-2 text-center font-black ${
                          res === 'wk' ? 'text-p5-red' :
                          res === 'rs' ? 'text-white' :
                          res === '-' ? 'opacity-10' : 'text-yellow-500'
                        }`}>
                          {res === '-' ? '-' : res.toUpperCase()}
                        </td>
                      ))}
        ...
        <div className="space-y-6">
          <div className="p5-card border-white/10">
            ...
            <div className="space-y-4 text-[10px] font-bold uppercase italic tracking-tighter">
              ...
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-white/40">Démons du Trésor</span>
                <span>{personas.filter(p => p.rare).length}</span>
              </div>
            </div>
          </div>

          <div className="p-4 bg-p5-red/10 border-2 border-p5-red italic text-[10px] leading-relaxed">
            <span className="font-black text-p5-red block mb-1 underline">AIDE AU COMBAT</span>
            Utilisez l'icône <Info className="inline" size={10} /> pour voir les stats détaillées (Force, Magie, etc.) et la liste complète des compétences héritables.
          </div>
        </div>
      </div>
    </div>
